                ...
                {data.map((project, index) => (
                    <div key={index} className="p-4 border border-gray-200 rounded-lg space-y-3">
                        ...
                        <div className='flex gap-2 w-full mt-2'>
                            <input value={project.codeUrl || ""} onChange={(e) => updateProject(index, "codeUrl", e.target.value)} type="url" placeholder="Code URL (e.g., GitHub repo)" className="px-3 py-2 text-sm rounded-lg flex-1" />
                            <input value={project.hostedUrl || ""} onChange={(e) => updateProject(index, "hostedUrl", e.target.value)} type="url" placeholder="Hosted URL (Live link)" className="px-3 py-2 text-sm rounded-lg flex-1" />
                        </div>
